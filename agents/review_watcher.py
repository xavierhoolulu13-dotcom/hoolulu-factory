"""Review Watcher agent for the Hoolulu Factory.
...
def run(lead):
    ...
    return {
        "notes": ((lead.get("notes") or "") + " | handled by review_watcher").strip(" |"),
...
def run_all(status="SCORED"):
    ...
    try:
        ...
        conn.close()
    return {"agent": "Review Watcher", "processed": len(results), "results": results}
