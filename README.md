...
**Hoolulu Offline** — the installable PWA (`hoolulu_offline/`) plus the
CodingAgent / AgentForge extension. Documented at the bottom of this file.

```bash
python -m amanda "build me a snake game and host it"   # the factory
./start.sh                                             # the lead agent (:8000)
cd hoolulu_offline && python3 server.py --port 8080     # the offline PWA
```
...
## Coding agent extension + Hoolulu Offline
